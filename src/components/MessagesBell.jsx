...
// Same live Firestore feed as NotificationBell, filtered to 'message' type
// items only, so unread message counts get their own icon like the mockup.
export default function MessagesBell({ items }) {
  const { user } = useAuth();
  const navigate = useNavigate();
  const [open, setOpen] = useState(false);
  const ref = useRef(null);

  const messages = items.filter((n) => n.type === 'message');
      ...
      {open && (
        <div className="absolute right-0 mt-2 w-80 bg-white border border-slate-200 rounded-xl shadow-lg z-50 overflow-hidden">
          ...
          <div className="max-h-80 overflow-y-auto">
            {messages.length === 0 && (
              <p className="text-sm text-slate-400 px-4 py-6 text-center">No messages yet</p>
            )}
            {messages.map((n) => (
              <button
                key={n.id}
